<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){ // Valido que la información del formulario haya sido enviada por medio del método POST
        if(isset($_POST['nombre_libro']) && !empty($_POST['autor_libro'])){ // Valido que el campo nombre_autor no esté vacío y que exista. Ojo, solo valido con nombre autor ya que la nacionalidad no es un campo obligatorio
            $nombre_libro = $_POST['nombre_libro']; // Obtengo el valor del campo nombre_libro del formulario
            $autor_libro = $_POST['autor_libro']; // Obtengo el valor del campo autor_libro del formulario
            ...
            $stmt = $conn->prepare("INSERT INTO libros(titulo, autor_id) VALUES (?, ?)"); // Preparamos la consulta SQL utilizando el método prepare(). Los signos de interrogación, son marcadores de posición para los valores que se van a insertar.

            $stmt->bind_param("ss", $nombre_libro, $autor_libro); // Enlazamos los parámetros utilizando el método bind_param(). La función bind_param() toma dos parámetros: El primero indica el tipo de dato de los parámetros, para este ejemplo usamos "ss" ya que ambos parámetros son de tipo string. El segundo y tercer parámetro son las variables que contienen los valores que se van a insertar en la base de datos. Estos son valores posicionales, es decir que si inviertes el orden de alguno, el valor se insertará en la columna incorrecta.
